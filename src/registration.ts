...
const formatterFactory = memoize(function formatterFactory(format: string) {
  const dynamicFormats = format
    .split("|")
    .map(format => DynamicFormat.parse(format));
  const orderedFormats = dynamicFormats.sort(
    (lhs, rhs) => rhs.threshold - lhs.threshold
  );

  return function dynamicFormatter(value: number) {
    let matching =
      orderedFormats.find(dynamic => value >= dynamic.threshold) ||
      dynamicFormats[0];
